...
public class AnnotationConfigApplicationContext extends GenericApplicationContext implements AnnotationConfigRegistry {
	...
	/**
	 * 顾名思义是一个reader，一个读取器
	 * 读取一个被加了注解的bean，实在类的默认构造方法中实例化的
	 */
	private final AnnotatedBeanDefinitionReader reader;

	//保存一个扫描指定类路径中注解BeanDefinition的扫描器，并将其设置到容器中
	/**
	 * 同样，顾名思义，是一个扫描器，扫描所有加了注解的bean
	 */
	 ...
	 * 默认构造方法
	 * 初始化一个bean的读取和扫描器
	 * 如果直接调用这个默认的构造方法，需要在稍后通过调用其 register()
	 * 去注册配置类（java configure），refresh 方法刷新容器
	...
	/**
	 * 这个构造方法需要转入一个被java configure 注解了的类，然后会把这个被java configure的类
	 * 通过注解读取器读取后，再解析
	...
	public AnnotationConfigApplicationContext(Class<?>... annotatedClasses) {
		//这里他先调用父类构造方法，然后在自己的构造方法中初始化一个读取器和扫描器
		this();
	 ...
	 */
	//该构造函数会自动扫描以给定的包及其子包下的所有类，并自动识别所有的SpringBean，将其注册到容器中
	...
	//---------------------------------------------------------------------
	// (1).直接将注解Bean注册到容器中：可以在初始化容器时注册；
	// 		也可以在容器创建之后手动调用注册方法向容器注册，然后通过手动刷新容器，
	// 		使得容器对注册的注解Bean进行处理。
	// (2).通过扫描指定的包及其子包下的所有类：
	//		在初始化注解容器时指定要自动扫描的路径，如果容器创建以后向给定路径动态添加了注解Bean，
	//		则需要手动调用容器扫描的方法，然后手动刷新容器，使得容器对所注册的Bean进行处理。
	//----------------------------------------------------------------------
	...
	//---------------------------------------------------------------------
	// Convenient methods for registering individual beans  注册单个bean的便捷方法
	//---------------------------------------------------------------------
